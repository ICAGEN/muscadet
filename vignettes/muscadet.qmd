...
`muscadet` (**mu**ltiomics **s**ingle-cell **c**opy number **a**lterations **det**ection) is an R package for identifying copy number alterations (CNAs) in cancer cells from single-cell multiomics data.
...
To adjust filters applied to features by `computeLogRatio()`, you can check distribution of data and filter status of features.
