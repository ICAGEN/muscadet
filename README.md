...
> Denoulet M, Giordano N, Minvielle S, Vallot C, Letouzé E (2026). _muscadet:
Multiomics Single-Cell Copy Number Alterations Detection_. R package version
0.2.2, https://icagen.github.io/muscadet/, <https://github.com/ICAGEN/muscadet>.
...
Sep 2024, Turku, Finland. [⟨hal-05150028⟩](https://nantes-universite.hal.science/hal-05150254v1)
...
accuracy and resolution of CNA detection within tumoral subclones.
The *muscadet* framework enables integration of joint multiomics data, such as
single-cell transcriptomics (scRNA-seq) and chromatin accessibility
(scATAC-seq), captured from the same cells. Cells are initially clustered by
relative coverage (log ratio of read counts) integrated across multiple omics
layers. CNA calling is then performed on aggregated coverage and allelic (read
counts per allele) data at the cluster level, leveraging combined signals from
omics and cells. Lastly, *muscadet* includes visualization tools for genome-wide
coverage assessment, subclone identification, and comprehensive CNA profiling.
